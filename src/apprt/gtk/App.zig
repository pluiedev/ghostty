...
/// Read method, instead of the recommended ReadOne method.
dbus_retry: bool = true,
    ...
    if (c.g_dbus_connection_call_finish(dbus, res, &err)) |value| {
        if (c.g_variant_is_of_type(value, c.G_VARIANT_TYPE("(v)")) == 1) {
            ...
            if (c.g_variant_is_of_type(inner, c.G_VARIANT_TYPE("u")) == 1) {
                self.colorSchemeEvent(if (c.g_variant_get_uint32(inner) == 1) .dark else .light);
                return;
            }
        }
    } else if (err) |e| {
        ...
        if (self.dbus_retry and e.code == 19) {
            c.g_dbus_connection_call(
                dbus,
                "org.freedesktop.portal.Desktop",
                "/org/freedesktop/portal/desktop",
